...
$orthologs_file and $source_gtf or die "usage: $0 one2one_orthologs_wga.txt source.gtf\n";
...
while (my $line = <$fh>) {
    ...
    if (not exists $genomic_alignments{$source_gene}) {
        $genomic_alignments{$source_gene} = [];
    }
...
while (my $line = <$fh>) {
    chomp $line;
    next if ($line =~ m/^#/);
    my ($chr, $method, $feature, $start, $end, $score, $strand, $phase, $attributeString) = split /\t/, $line;
    ...
    my ($newChr, $newStart, $newEnd, $newStrand) = convert($chr, $start, $end, $strand, $genomic_alignments{$gene_id});
    if (not $newChr) {
      $failed{$gene_id} = 1;
    }
    else {
      push @mapped, [$gene_id, $newChr, $method, $feature, $newStart, $newEnd, $score, $newStrand, $phase, $attributeString];
    }
}
close $fh;

for my $interval (sort {$a->[1] cmp $b->[1] or $a->[4] <=> $b->[4] or $b->[5] <=> $a->[5]} @mapped) {
...
sub convert {
    ...
    for my $alignment (@$alignments) {
        my ($source_region, $source_start, $source_end, $source_strand, $source_cigar,
            $target_region, $target_start, $target_end, $target_strand, $target_cigar) = @$alignment;
        if ($start <= $source_end and $start >= $source_start) {
            # found interval covering $start
            my $align_pos = map_seq_pos_to_alignment_pos($start - $source_start, $source_cigar);
            $newStart = map_alignment_pos_to_seq_pos($align_pos, $target_cigar, $target_start, $target_end, $target_strand);
            if ($newStart >= 0) {
        ...
        if ($end <= $source_end and $end >= $source_start) {
            # found interval covering $end
            my $align_pos = map_seq_pos_to_alignment_pos($end - $source_start, $source_cigar);
            $newEnd = map_alignment_pos_to_seq_pos($align_pos, $target_cigar, $target_start, $target_end, $target_strand);
            $regions{$target_region} = 1 if ($newEnd >= 0);
        }
    }
    my @newRegions = keys %regions;
    if ($newStart == -1 or $newEnd == -1 or @newRegions > 1) { # failed to map both ends uniquely
      return ();
    }
    if ($newStart < $newEnd) {
        return ($newChr, $newStart, $newEnd, $newStrand);
    } else {
